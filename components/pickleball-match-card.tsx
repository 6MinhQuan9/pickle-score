        ...
        <div className="flex items-stretch justify-between">
          ...
          <div className="flex flex-col items-center justify-center w-1/3">
            <div className="flex items-center justify-center gap-1">
              <span className={cn("text-xl font-bold tabular-nums", team1Won && "text-primary")}>{match.score.team1}</span>
              <span className="text-xl font-bold text-muted-foreground">-</span>
              <span className={cn("text-xl font-bold tabular-nums", team2Won && "text-primary")}>{match.score.team2}</span>
            </div>
            <span className="text-xs font-medium mt-1">{formattedDate}</span>
          </div>
...
function TeamSection({ players, isWinner }: TeamSectionProps) {
  return (
    <div className="flex flex-col items-center w-1/3">
      <div className={cn("flex -space-x-2 transition-transform mb-3", isWinner ? "scale-105" : "scale-100")}>
        {players.map((player, index) => (
          <Avatar
            key={index}
            className={cn(
              "w-8 h-8 border-2 border-background transition-all duration-300",
              isWinner ? "ring-2 ring-primary ring-offset-1" : "ring-2 ring-muted ring-offset-1",
              "hover:scale-110 hover:z-10",
            )}
          >
            <AvatarImage src={`/${player.avatarUrl}`} alt={player.username} className="object-cover" />
