...
export function MatchForm({ players, onSubmit }: MatchFormProps) {
  ...
  const [sets, setSets] = useState<Array<{ team1Score: number, team2Score: number }>>([
    { team1Score: 0, team2Score: 0 }
  ])
  const { toast } = useToast()

  const handleSetScoreChange = (setIndex: number, team: 1 | 2, value: string) => {
    console.log(value)
    const newSets = [...sets]
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate scores
    if (sets.some(set => 
      (set.team1Score === undefined || set.team2Score === undefined) ||
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="p-4 bg-muted/30 rounded-lg space-y-4">
              <h2 className="text-lg font-semibold border-b pb-2">Match Type</h2>
              <RadioGroup
                value={matchType}
                      ...
                      <div className="flex-grow grid grid-cols-2 gap-2">
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => handleSetScoreChange(index, 2, e.target.value)}
                          />
                        </div>
                      </div>
                    </div>
