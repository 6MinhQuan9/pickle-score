...
import { Match, Player, DatabaseMatch, MatchResult } from '@/types'
...
export async function getMatches(): Promise<Match[]> {
  ...
  return (data as DatabaseMatch[]).map(match => {
    ...
    const team1SetsWon = team1.scores.filter(s1 => {
      const team2Score = team2.scores.find(s2 => s2.set_number === s1.set_number)
      return s1.score > (team2Score?.score || 0)
    }).length

    const team2SetsWon = team2.scores.filter(s1 => {
      const team1Score = team1.scores.find(s2 => s2.set_number === s1.set_number)
